...
- One H2 DB, file-backed at `backend/data/appdb` (`ddl-auto: update`) so data survives restarts. Prod overrides the URL/ddl-auto via `SPRING_DATASOURCE_URL`/`SPRING_JPA_HIBERNATE_DDL_AUTO` env vars to its own persistent volume. Table names are namespaced per domain already (`app_users`, `posts`, `photos`/`videos`, `visited_places`, `guest_entries`, `journal_entries`, `visit_logs`) so they share one schema with no collisions. `USER` is reserved in H2 2.x, hence `app_users`.
- **No demo-data seeders.** content/media/travel/guestbook used to ship with a `DataSeeder` that inserted placeholder rows on first boot — that code has been deliberately deleted (2026-08-13) and the matching seed rows purged from production. **Do not re-add a seeder for these domains** — production is meant to hold only real content the owner adds through the UI. (`auth.config.OwnerSeeder` is the one exception: it still seeds exactly one owner user from env vars on an empty `app_users` table, since the app needs an account to log in with at all.)
